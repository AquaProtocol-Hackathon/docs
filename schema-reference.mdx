...
description: "Complete reference for Aqua Protocol schema definitions and structures"
icon: "database"
---

# Schema (v3.2)

The schema documentation below outlines the five types of revisions (file, content, signature, witness, link and form) and their respective properties.

This schema defines a structure for atomic revisions of aqua secured files. Each revision is uniquely identified by a hash and contains metadata specific to its type.

## Overview

<Info>
The revision is identified by a unique hash which is also used to verify the integrity of the revision. It acts as a globally unique identifier. Unique identifiers are used as unique references to link revisions together or to perform operations on them (transport, access control, etc.).
</Info>

## Revision Types

There are **five main types of revisions**:

1. **Content Revision** (1.1 Form Revision - a special type of content revision for layer 2 applications)
...
All revision types share properties which are common forming a hash-chain:
...
The hashing method used is **SHA256**. The revision object sorts all key-values as 'Canonical JSON' this ensures no whitespaces, sorted keys, no floating-point ambiguity and unicode normalization.

### Hashing Types

<CardGroup cols={2}>
  <Card title="Scalar" icon="hash">
    Simple hash of the stringified revision object. This is the default method for performance reasons.
  </Card>
  
  <Card title="Tree" icon="tree">
    Creates a merkle tree of all the values. This allows a more granular verification and is used for selective disclosure.
  </Card>
</CardGroup>
...
<Warning>
The system is designed to allow for the hash algorithm to be exchangeable. Same applies to any other cryptographic ciphers being used (e.g. signatures) to ensure post-quantum security. Any type is modular, allowing for new subtypes to be introduced with different ciphers.
</Warning>

## 1. Content Revision

A Content Revision represents the initial or updated state of a file which is secured by the Aqua-Protocol. It includes metadata about the file: its hash and index. The index is a mutable part of the data structure, it allows to update file location and the services which handle the file.
...
- `local_timestamp`: (String) The UTC timestamp of the revision in YYYYMMDDHHMMSS format. It's a local timestamp of the server running the aqua-service. For cryptographically secured timestamps the witness revision is used.
- `revision_type`: (String) The type of revision, always "content" for this type.
- `file_hash`: (String) The hash of the file content.
- `file_nonce`: (String) A unique nonce associated with the file. This is used to ensure that additional entropy is added to the hash to make it globally unique.
- `version`: (String) The schema version and hashing method used.
...
Form revisions are a special type of content revision for layer 2 applications. They are used to create a form that can be signed by the user.

<Info>
Native Layer 2 applications are aqua identity and aqua access control. The tree verification structure allows for selective disclosure of the form data.
</Info>

Find detailed examples under the "Aqua Identity" and "Access Control" tabs.

### Example Form Revision
...
- `signature_type`: (String) The type of signature (e.g., "ethereum:eip-191"). This allows for different signature types to be added to Aqua Protocol.
- `version`: (String) The schema version and hashing method used.

### Example Signature Revision
...
A Link Revision represents a record of other revisions belonging to other or the same AquaTree structure. This serves two distinct purposes:

1. To be able to reference other AquaTree objects in a verifiable way.
2. To acknowledge forks within an AquaTree which could be removed without recognition without a link revision to "merge" those revisions into the continuous hash-chain structure.
...
The `file_index` object maps revision hashes to file names in a mutable form. This allows for files to be reorganized or to be associated with various services. As the revision is immutable, it is required that this is external to the revision.
...
Below is a complete example of how an AquaTree looks:
